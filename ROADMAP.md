...
- ✅ **`contextIsolation` / `nodeIntegration`** — **resuelto.** Verificado en produccion: ambas ventanas (index y chat) corren con `contextIsolation: true`, `nodeIntegration: false` y `webSecurity: true`, con preloads acotados (`src/preload.js`, `src/chat/preload.js`) que exponen SOLO `window.assistant` vía `contextBridge`. El renderer ya no usa `require()` directo. [implementado]
- ✅ **`sandbox` del renderer (chat)** — **resuelto (refactor del preload).** La ventana de chat pasa a `sandbox: true` de Electron: el preload quedó fino (solo `contextBridge` + `ipcRenderer` con allowlists) y toda la lógica Node se movió a `ipc/chat-handlers.js` (comandos, LLM con abort, core-sources, fs, TTS, `FileResolver`, `AgentManager`); `renderMarkdown` (marked) + sanitización (DOMPurify) corren en la página. Verificado: e2e UI 22/22, suite 2358/2358. **Alcance:** es el sandbox del renderer (`webPreferences.sandbox`), no `contextIsolation` — mecanismos distintos; `contextIsolation` sigue en `true`. El overlay Live2D conserva su postura (`webSecurity:false` como tradeoff documentado por sus CDNs). [implementado]
- ✅ **Límite de confianza anti prompt-injection (P3)** — el contenido de terceros (páginas vía browser `get_text`, body de `webfetch`, snippets de `web_search`/`websearch`) entra al contexto del LLM delimitado como `<contenido_no_confiable>` con nota al modelo y con los patrones de inyección neutralizados (`core/grounding/untrustedContent.js`, aplicado en `BrowserBridge.js` y `openclaw-server.js`). Suite `test_untrusted_content` 43/43. [implementado]
- ✅ **Env limpio para hijos del openclaw-server (P2)** — `exec`/`code_execution` ya no heredan secrets del `process.env` de la app (`_safeChildEnv`: conserva PATH/HOME/LANG, elimina KEY/TOKEN/SECRET/PASSWORD/PAT/AUTH/CREDENTIALS/AWS_ACCESS_KEY_ID...). Defensa en profundidad: el control real sigue siendo la aprobación humana de `isHighImpact`, el blocklist por regex sigue siendo evadible por diseño. [implementado]
- ✅ **Validación de scopes OAuth de GitHub (P4)** — `OAuthDeviceFlow.validateScopes` verifica que el token devuelto cubra el mínimo `repo read:user`; `/github login` avisa por chat si el usuario desmarcó permisos (sin `repo`, las tools de issues/PRs fallarán; whoami/lecturas públicas siguen funcionando). [implementado]
...
- **v3 → hoy (§10, CI verde):** Pipeline de GitHub Actions completamente en verde. Fix CI para el runner **Ubuntu 24.04 (noble)**: `libasound2` (paquete virtual) → `libasound2t64`, actions a v5 (checkout/setup-node/upload-artifact). `git_push` validado en CI (`git push -u` para rama sin upstream). `test_signal_sensors` hermético: el merge real abortaba en CI con "Committer identity unknown" (git 2.43 sin `user.email` global; local 2.55 auto-detecta) → `makeRepo()` fija identidad a nivel de repo. Verificado reproduciendo en contenedor `ubuntu:24.04` (git 2.43.0, 49/49) y local (49/49); regresión local 1389 passed · 0 failed. Push real del §10 a produccion (`0a4d638`), ya sincronizado con el remoto.
- **v3 → hoy (§10, conexión):** Comando `/github` con **OAuth Device Flow (RFC 8628)**: `login` (sin arg) usa el `client_id` guardado con `client-id <ID>` o `.env` (`GITHUB_CLIENT_ID`), abre el navegador y hace polling en background con notificación por chat — siempre muestra el código `XXXX-YYYY` para las OAuth Apps nuevas (Client ID `Ov23...` que piden tipearlo); `login <PAT>` valida contra la API antes de persistir y nunca filtra el token; `whoami`; `logout`; `status`. Token en llavero (`github_token`) y memoria; sesión efímera con aviso si el llavero no está. `GitHubManager.configure` acepta `token: null` para limpiar memoria en logout y `resolveToken` inyectable (tests herméticos). CORS del renderer resuelto: `core/github/net.js` (proxy `github-fetch` por IPC al main) usado por GitHubManager/OAuthDeviceFlow; `ipc/github-handlers.js` registra `github-fetch` + `github-client-id`. Tests `test_github_command` 37/37 + `test_oauth_device_flow` 16/16; regresión 1296 passed (2 fallos ambientales pre-existentes). Listo para distribuir: token solo en main/keychain, sin fallback a texto plano.
- **v3 → hoy (§10):** Git nativo (`core/git/GitManager.js`): `git_status/diff/log/branch` (lectura) + `git_commit/stash/merge/rebase` (mutadores con aprobación y conflictos estructurados) + **`git_push`** (sube al remoto usando el token de GitHub conectado `github_token` vía `GIT_ASKPASS` temporal — el token nunca va en argv; rechaza remotos/ramas inválidos y da hint si GitHub deniega), salida JSON vía `execFile` (sin shell). GitHub nativo (`core/github/GitHubManager.js`): `repo_info`, `issue_list/create/comment/close`, `pr_list/create/review`, `actions_status` con PAT via KeychainManager (`github_token`) y transport `fetch` inyectable. Dispatch en AgentLoop (sets `GIT_TOOLS`/`GITHUB_TOOLS`, patrón LSP); aprobación en `ActionParser.isHighImpact`; schemas en catálogo (ToolRegistry), nativos LLM (ToolSchemas) y resolución (ToolResolver). Tests: `test_git_manager` 38/38 (incl. push a bare repo), `test_github_manager` 25/25, `test_agent_loop_git` 53/53; regresión 1389 passed 0 failed (todas en verde). MCP de GitHub queda como opción opt-in sin código nuevo.
