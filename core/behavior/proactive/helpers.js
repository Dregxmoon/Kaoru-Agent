...
function _triggerDescription(trigger) {
  ...
  switch (trigger.type) {
    ...
    case 'goal_ready':
      return `Hay un objetivo pendiente y ejecutable en el workspace actual: "${String(trigger.goal || '').slice(0, 240)}". ${trigger.reason === 'permission_required' ? 'Aún falta una autorización explícita para ejecutarlo de forma autónoma.' : 'Ofrece retomarlo desde el siguiente paso comprobado.'} No afirmes que ya avanzaste ni mezcles otro proyecto.`;
    case 'git_redflag':
