...
module.exports = {
  ...
  async _buildEmotionalContext(trigger) {
    try {
      ...
      try {
        ...
        const sessionId =
          typeof this._getCurrentSessionId === 'function' ? this._getCurrentSessionId() : null;
  ...
  _formatEmotionalContext(emotions) {
    try {
      ...
      const dominant = Object.entries(emotions)
        .filter(
          ([k, v]) =>
            typeof v === 'number' && v > 0.4 && !['tone', 'energy', 'implicitIntent'].includes(k)
        )
        .sort((a, b) => b[1] - a[1])
        .slice(0, 2);
      ...
      if (emotions.frustration > 0.6) {
        parts.push(
          'INSTRUCCIÓN: El usuario está frustrado. Sé breve, empática y directa. No preguntes "¿cómo va?" — ve al grano.'
        );
      }
      if (emotions.enthusiasm > 0.6) {
        parts.push(
          'INSTRUCCIÓN: El usuario está entusiasmado. Celebra con él, muestra interés genuino.'
        );
      }
      if (emotions.confusion > 0.5) {
        parts.push(
          'INSTRUCCIÓN: El usuario está confundido. Explica de forma simple, paso a paso.'
        );
      }
  ...
  _buildTopicContext(trigger) {
    try {
      ...
      if (trigger?.type === 'topic_hot' && trigger.topic) {
        parts.push(
          `El usuario acaba de mencionar "${trigger.topic}" que es un topic caliente — conéctalo con algo de memoria.`
        );
      }

      if (trigger?.type === 'topic_cold' && trigger.topic) {
        parts.push(
          `El usuario no ha hablado de "${trigger.topic}" en un tiempo. Si es natural, retómalo.`
        );
      }
  ...
  _recordKaoruResponse(kaoruResponse, enforcement, emotionalCtx, adaptationType) {
    try {
      ...
      graph._responseEvaluator.recordResponse(
        kaoruResponse,
        enforcement,
        emotionalCtx,
        adaptationType
