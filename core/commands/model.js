  ...
  register({
    ...
    handler: async (args, ctx) => {
      ...
      try {
        ...
        if (total === 0)
          return `El modelo **${info.name}** no tiene expresiones ni animaciones para mostrar.`;
        ...
        if (first === 'test') {
          ...
          if (!q)
            return 'Uso: `/gestos test <gesto o emocion>`\nEjemplos: `/gestos test angry`, `/gestos test 哭`, `/gestos test zhaiyan`, `/gestos test tired`.';
          if (!ctx.gestureEngine)
            return 'El motor de gestos aun no esta listo — espera a que cargue el modelo en el panel.';
          ...
          return `No encontre un gesto para \`${q}\`. Disponibles para probar: ${names
            .slice(0, 8)
            .map((n) => `\`${n}\``)
            .join(', ')}${names.length > 8 ? '…' : ''}.`;
        }
        ...
        lines.push(
          '',
          `Prueba: \`/gestos test 哭\`, \`/gestos test angry\`, \`/gestos test sleepy\`.`
        );
        return lines.join('\n');
      } catch (e) {
        return `Error: ${e.message}`;
