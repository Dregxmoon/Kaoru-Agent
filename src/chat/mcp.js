...
async function refreshMcpBadge() {
  ...
  } catch(e) { console.error('[mcp] error al listar servidores:', e.message || e); }
}
...
async function renderMcpServerList() {
  ...
  let servers = [];
  try { servers = await ipcRenderer.invoke('mcp-list-servers'); } catch(e) { console.error('[mcp] error al listar servidores:', e.message || e); }
