...
const PORT =
  parseInt(process.env.KAORU_TOOL_HOST_PORT || process.env.OPENCLAW_PORT, 10) || DEFAULT_PORT;

// ── Configuración desde entorno ─────────────────────────────────────────────

const API_KEY = process.env.KAORU_TOOL_HOST_API_KEY || process.env.OPENCLAW_API_KEY || null;
const ALLOWED_PATH = process.env.KAORU_TOOL_HOST_ALLOWED_PATH
  ? path.resolve(process.env.KAORU_TOOL_HOST_ALLOWED_PATH)
  : process.env.OPENCLAW_ALLOWED_PATH
...
if (require.main === module && !API_KEY) {
  logger.error('kaoru-tool-host', 'KAORU_TOOL_HOST_API_KEY no definida — abortando');
  process.exit(1);
}
...
const AUDIT_LOG_PATH =
  process.env.KAORU_TOOL_HOST_AUDIT_PATH ||
  process.env.OPENCLAW_AUDIT_PATH ||
  path.join(require('os').tmpdir(), 'kaoru-tool-host-audit.jsonl');
  ...
  try {
    ...
    if (process.platform === 'win32') {
      ...
      _sandboxInitPromise = _windowsSandbox.initialize().then((enabled) => {
        ...
        if (enabled) logger.info('kaoru-tool-host', 'sandbox de proceso: AppContainer habilitado');
        else
          logger.error('kaoru-tool-host', `sandbox AppContainer no disponible: ${_sandboxReason}`);
      });
      return;
    }
    ...
    _sandboxKind = 'bwrap';
    logger.info('kaoru-tool-host', 'sandbox de proceso: bwrap habilitado');
  } catch (e) {
...
function _authenticate(headers) {
  ...
  // X-Api-Key se conserva para compatibilidad con clientes anteriores.
...
async function startServer(port = PORT) {
  ...
  return new Promise((resolve, reject) => {
    server.listen(port, '127.0.0.1', () => {
      logger.info('kaoru-tool-host', `escuchando en http://127.0.0.1:${port}`);
      logger.info('kaoru-tool-host', `allowed path: ${ALLOWED_PATH}`);
      logger.info('kaoru-tool-host', `auth: ${API_KEY ? 'enabled' : 'disabled'}`);
      resolve(server);
    });
...
if (require.main === module) {
  startServer(PORT).catch((e) => {
    logger.error('kaoru-tool-host', `error al iniciar: ${e.message}`);
    process.exit(1);
  });
