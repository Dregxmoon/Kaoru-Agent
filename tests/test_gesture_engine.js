...
function makeModel() {
  ...
  return {
    calls,
    motion: async (g, i, p) => {
      calls.push(['motion', g, i, p]);
      return true;
    },
    expression: async (n) => {
      calls.push(['expression', n]);
      return true;
    },
    internalModel: {
      motionManager: {
        expressionManager: {
          resetExpression() {
            calls.push(['resetExpression']);
          },
        },
        stopAllMotions() {
          calls.push(['stopAllMotions']);
        },
      },
      coreModel: {
        getParameterCount() {
        ...
        getParameterDefaultValue(i) {
          return i * 10;
        },
        setParameterValueByIndex(i, v, w) {
          calls.push(['param', i, v, w]);
        },
      },
    },
  };
...
(async () => {
  ...
  const engine = new GestureEngine({
    config: { minIntervalMs: 2500, cooldownMs: 15000, durationMs: 50 },
  });
  engine.attach(model, { model3Path: FIXTURE });

  let r = await engine.play('angry');
  assert(
    r.ok && r.gesture.name === 'angry',
    'play(angry) aplica la expresión "angry"',
    JSON.stringify(r)
  );
  assert(
    model.calls[0][0] === 'expression' && model.calls[0][1] === 'angry',
    'se llamó model.expression("angry")'
  );
  ...
  assert(
    !r.ok && (r.reason === 'min-interval' || r.reason === 'cooldown'),
    'mismo mood dentro del cooldown → skip',
    JSON.stringify(r)
  );
  r = await engine.play('happy');
  assert(
    !r.ok && r.reason === 'min-interval',
    'otro mood dentro del min-interval → skip',
    JSON.stringify(r)
  );
  r = await engine.play('happy', { priority: 'force' });
  assert(
    r.ok && r.gesture.name === 'Feliz',
    'force ignora cooldowns → happy/Feliz',
    JSON.stringify(r)
  );
  ...
  assert(
    (typeof r === 'object' && !r.gesture) || r.gesture,
    'mood inexistente forzado no crashea ni recicla',
  ...
  assert(
    resets >= 1,
    'tras durationMs se revierte la expresión (resetExpression + params)',
    `resets=${resets}`
  );
  assert(
    !model.calls.some((c) => c[0] === 'stopAllMotions'),
    'el revert de expresión no corta motions (idle intacto)',
  ...
  assert(
    model.calls.some((c) => c[0] === 'motion' && c[1] === 'motions'),
    'motion "sing" → model.motion("motions", index)'
  );
  model.calls.length = 0;
  await sleep(80);
  assert(
    model.calls.some((c) => c[0] === 'stopAllMotions'),
    'la motion en bucle se corta al revertir',
    JSON.stringify(model.calls)
  );
  assert(
    model.calls.some((c) => c[0] === 'param'),
    'se restaura la pose neutra (params a default)'
  );
  ...
  assert(
    !r.ok && r.reason === 'sdk-rechazo',
    'modelo sin motion/expression → sdk-rechazo',
    JSON.stringify(r)
  );
  ...
  const engine3 = new GestureEngine({
    config: { minIntervalMs: 0, cooldownMs: 0, durationMs: 5000 },
  });
  engine3.attach(model3, { model3Path: FIXTURE });
  await engine3.setEmotion('happy');
  assert(
    model3.calls.some((c) => c[0] === 'expression' && c[1] === 'Feliz'),
    'setEmotion(happy) → Feliz'
  );
  ...
  assert(
    model3.calls.some((c) => c[0] === 'expression' && c[1] === 'Feliz'),
    'onEvent(command_ok) → happy → Feliz'
  );
  model3.calls.length = 0;
  ...
  assert(
    model3.calls.length === 0,
    'onEvent(proposal-result, ok:false) → sad sin gesto → no anima y no crashea',
    JSON.stringify(model3.calls)
  );

  // ── Test 8: attach por model3Path y stats ──────────────────────────────────
  console.log(C.bold('\n── Test 8: stats y detach ─────────────────────────────────'));
  const engine4 = new GestureEngine({
    config: { minIntervalMs: 0, cooldownMs: 0, durationMs: 10 },
  });
  ...
  assert(
    typeof stats.plays === 'number' && typeof stats.skipped === 'object',
    'stats tiene contadores',
