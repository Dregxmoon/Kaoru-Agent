...
async function runTests() {
  ...
  try {
    ...
    } = require('../core/state-graph/evolution/CommunicationStyleProfiler.js');
    const {
      TopicMomentumTracker,
      extractTopics,
    } = require('../core/state-graph/evolution/TopicMomentumTracker.js');
    const {
    ...
    const longLengthScore = STYLE_METRICS._measureLength(
      'Este es un mensaje muy largo con mucho contenido para probar la medición de longitud en el sistema de aprendizaje evolutivo del asistente'
    );
    ...
    assert(
      STYLE_THRESHOLDS.length.brief < STYLE_THRESHOLDS.length.normal,
      'Brief threshold should be less than normal'
    );
    assert(
      STYLE_THRESHOLDS.length.normal < STYLE_THRESHOLDS.length.detailed,
      'Normal threshold should be less than detailed'
    );
    ...
    assert(
      DEFAULT_PROFILE.technicalLevel === 'moderate',
      'Default technical level should be moderate'
    );
    ...
    const {
      ...
      TOPIC_WINDOW_MS,
      MAX_TRACKED_TOPICS,
    } = require('../core/state-graph/evolution/EvolutionStore.js');
