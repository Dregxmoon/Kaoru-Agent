...
assert(
  expNames.length === 2,
  'solo 2 expresiones (referenciada + descubierta; sub/1.exp3.json deduplicado por basename)',
  JSON.stringify(expNames)
);
...
assert(
  g.motions.length === 3,
  '3 motions (Idle Animation + sing + wave)',
  JSON.stringify(motNames)
);
...
assert(
  wave && wave.group === 'motions' && wave.index === 1,
  'wave → grupo motions, index 1',
  JSON.stringify(wave)
);
...
assert(
  typeof settings.url === 'string' && settings.url.startsWith('file:///'),
  'settings.url presente',
  settings.url
);
assert(
  Array.isArray(settings.FileReferences.Expressions) &&
    settings.FileReferences.Expressions.length === 2,
  'FileReferences.Expressions inyectadas'
);
assert(
  settings.FileReferences.Motions && Object.keys(settings.FileReferences.Motions).length === 2,
...
assert(
  gestures === ModelAugmenter.listGestures(FIXTURE),
  'cache devuelve el mismo objeto de gestos'
);
...
assert(
  r.ok && r.gesture.name === 'Idle Animation',
  'default → animación del grupo Idle',
  JSON.stringify(r)
);
...
assert(
  r.ok && r.gesture.name === 'sing' && r.source === 'config',
  'mapping config gana a la heurística',
  JSON.stringify(r)
);
...
assert(
  GestureHeuristic.scoreGesture({ name: 'white eyes', kind: 'expression' }, 'wave') === 0,
  '"hi" no puntúa sobre "white eyes"'
);
assert(
  GestureHeuristic.scoreGesture({ name: 'No Maidens', kind: 'expression' }, 'shake') === 0,
...
assert(
  all.map['default'] && all.map['default'].name === 'Idle Animation',
  'resolveAll: default→Idle Animation'
);
...
assert(
  gr.motions.length === 4,
  'motions referenciadas + descubierta entran al listado (4)',
  JSON.stringify(grMots)
);
const gIdle = gr.motions.find((m) => m.name === 'idle0');
assert(
  gIdle && gIdle.group === 'Idle' && gIdle.referenced === true,
  'motion referenciada bajo grupo "" se re-clasifica por nombre → Idle',
  JSON.stringify(gIdle)
);
const gMtn = gr.motions.find((m) => m.name === 'mtn_00');
assert(
  gMtn && gMtn.group === 'Idle' && gMtn.referenced === true,
  'motion referenciada bajo Motions.Idle preserva el grupo Idle (aunque el archivo no diga idle)',
  JSON.stringify(gMtn)
);
const gWave = gr.motions.find((m) => m.name === 'wave');
assert(
  gWave && gWave.group === 'motions' && gWave.referenced === true,
  'motion referenciada bajo grupo "" no-idle → motions',
  JSON.stringify(gWave)
);
assert(
  gr.motions.some((m) => m.name === 'extra' && m.referenced === false),
  'motion descubierta en disco también entra'
);
assert(
  !gr.motions.some((m) => m.name === 'idle0' && !m.referenced),
  'duplicado por basename (anim/sub/idle0) no entra dos veces'
);
const augRef = ModelAugmenter.augmentModel(GREF);
assert(
  augRef.settings.FileReferences.Motions.Idle &&
    augRef.settings.FileReferences.Motions.Idle.length === 2,
  'settings inyecta ambas motions idle (idle0 + mtn_00) bajo grupo Idle',
  JSON.stringify(augRef.settings.FileReferences.Motions)
);
