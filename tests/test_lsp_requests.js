...
function testWorkspaceConfiguration() {
  ...
  assert(
    Array.isArray(resp.result) && resp.result[0]?.includeCompletionsForModuleExports === true,
    'Devuelve la sección pedida de initializationOptions',
...
function testOtherRequests() {
  ...
  assert(
    sent.length === 2 && sent[1].result === null,
    'window/workDoneProgress/create responde null'
  );
  ...
  assert(
    unknown.error?.code === -32601 &&
      unknown.error?.message.includes('textDocument/unknownRequest'),
    'Request desconocida → MethodNotFound (-32601)',
...
async function testWaitForDiagnostics() {
    ...
    inst._handleMessage({
      ...
      params: {
        ...
        diagnostics: [
          {
            severity: 1,
            message: 'type X no es asignable',
  ...
  assert(
    Array.isArray(diags) && diags.length === 1 && diags[0].severity === 1,
    'Resuelve con los diagnósticos frescos',
...
async function testWaitForDiagnosticsTimeout() {
  ...
  inst._diagnostics.set(uri, [
    { severity: 1, message: 'viejo', range: { start: { line: 0, character: 0 } } },
  ]);
  ...
  assert(
    Array.isArray(diags) && diags.length === 1 && diags[0].message === 'viejo',
    'Timeout → devuelve la cache de push',
    JSON.stringify(diags)
  );
...
async function testPublishDiagnosticsCachesAndEmits() {
  ...
  inst._handleMessage({
    ...
    params: {
      uri,
      diagnostics: [{ severity: 2, message: 'warn', range: { start: { line: 1, character: 0 } } }],
    },
  });
  ...
  assert(
    Array.isArray(cached) && cached.length === 1 && cached[0].message === 'warn',
    'La cache push se actualizó'
  );
}
...
async function testHover() {
  ...
  assert(
    res && res.contents.includes('const x'),
    'Hover normalizado a texto plano',
    JSON.stringify(res)
  );
...
async function testRename() {
  ...
  let threw = null;
  try {
    await inst.rename(filePath, 0, 0, '');
  } catch (e) {
    threw = e;
  }
...
async function testCodeActions() {
  ...
  assert(
    Array.isArray(req.params.context.diagnostics) && req.params.context.diagnostics.length === 1,
    'Contexto incluye los diagnósticos cacheados',
    JSON.stringify(req.params.context)
  );
  ...
  assert(
    ...
    'Devuelve las code actions',
    JSON.stringify(actions)
  );
  assert(
    actions[0].title === 'Fix rápido' && actions[0].isPreferred === true,
    'Normaliza title/isPreferred'
  );
}
...
async function testRecoveryRestart() {
  ...
  assert(
    ...
    'Contador de intentos avanzó a 1',
  ...
  assert(
    startCalls === 2,
    '2º crash → segundo reinicio (backoff bajo el límite)',
  ...
  assert(
    startCalls === 2,
    'Cede tras el límite de intentos (no reinicia más)',
    `startCalls: ${startCalls}`
  );
  assert(
    crashed.length === 1 && crashed[0] === 'typescript',
    'Emite el evento crashed',
...
async function testRecoveryStableReset() {
  ...
  assert(
    startCalls === 1,
    'Crash de un server estable → se vuelve a reiniciar',
    `startCalls: ${startCalls}`
  );
  assert(
    inst._restartAttempts === 1,
    'Contador reseteado y re-arrancado (backoff desde el inicio)',
    `attempts: ${inst._restartAttempts}`
  );
}
...
async function testRecoveryStopCancels() {
  ...
  assert(
    startCalls === before,
    'Ningún reinicio se ejecutó tras stop',
    `startCalls: ${startCalls}`
  );
}
...
async function testAutoInstall() {
  ...
  try {
    ...
    assert(
      thrown !== null,
      'start falla: el comando no existe y no se instala solo',
      thrown?.message || ''
    );
    assert(installRuns === 1, 'auto-install corrió exactamente 1 vez', `installs: ${installRuns}`);
    assert(
      thrown && thrown.message.includes('comando-lsp-inexistente-xyz'),
      'El error final es el spawn del binario (sin reintentar otra vez)',
      thrown?.message || ''
    );
  } finally {
    try {
...
async function main() {
  ...
  console.log(
    C.bold(C.cyan('  Test Suite: LSP requests + recovery + auto-install + tools semánticas'))
  );
  console.log(C.bold(C.cyan('════════════════════════════════════════════════════════')));
