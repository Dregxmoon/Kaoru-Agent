...
function makeFakeKeychain(initial = {}) {
  ...
  return {
    ...
    setKey: (id, v) => { store[id] = v; return true; },
    deleteKey: (id) => { delete store[id]; return true; },
    _store: store,
  };
}
...
function main() {
  ...
  // ── 1. Sin llavero, la key de config se usa normal ─────────────────────
  {
    ...
    assert(p && p.apiKey === 'CONFIG-ONLY',
      'sin llavero, config.json sigue funcionando', JSON.stringify(p?.apiKey));
  ...
  // ── 2. El llavero gana sobre config.json ───────────────────────────────
  {
    ...
    const p = LLM.getAvailableProviders().find(x => x.id === 'groq');
    assert(p && p.apiKey === 'KEYCHAIN-KEY',
      'key del llavero tiene prioridad sobre config.json', JSON.stringify(p?.apiKey));
  }
  ...
  // ── 5. storeProviderApiKey / removeProviderApiKey contra el fake ───────
  {
    const fake = makeFakeKeychain({});
    LLM._setKeychainResolver(fake);
    assert(LLM.storeProviderApiKey('gemini', 'G-KEY') === true, 'storeProviderApiKey escribe en el llavero');
  ...
  // ── 6. migrateApiKeysToKeychain: mueve solo las ausentes y no pisa ─────
  {
    ...
    LLM._setKeychainResolver(fake);
    const r = LLM.migrateApiKeysToKeychain({ llm: {
      apiKeys: { groq: 'NUEVA', gemini: 'NUEVA2' },
      providers: { openai: { apiKey: 'NUEVA3' } },
    } });
    assert(Array.isArray(r.migrated) && r.migrated.includes('gemini') && r.migrated.includes('openai'),
      'migra las keys ausentes (gemini, openai)', JSON.stringify(r.migrated));
    assert(!r.migrated.includes('groq'), 'no pisa la key que ya existe en el llavero');
    assert(fake._store.gemini === 'NUEVA2' && fake._store.openai === 'NUEVA3',
  ...
  // ── 7. Sin llavero, migrate/store/remove no hacen nada y no rompen ─────
  {
    ...
    assert(r.migrated.length === 0 && r.keychainAvailable === false,
