...
async function testRepoInfo() {
  ...
  const fake = makeFetch({
    'GET https://api.github.com/repos/octo/repo': {
      body: {
        owner: { login: 'octo' },
        name: 'repo',
        full_name: 'octo/repo',
...
async function testIssues() {
  ...
  const fake = makeFetch({
    ...
    'POST https://api.github.com/repos/o/r/issues/2/comments': {
      status: 201,
      body: { id: 99, html_url: 'https://github.com/o/r/issues/2#issuecomment-99' },
    },
    'PATCH https://api.github.com/repos/o/r/issues/2': {
      body: { number: 2, state: 'closed', title: 'Nuevo' },
    },
  });
  ...
  assert(
    list.total === 1 && list.issues[0].number === 1 && list.issues[0].labels.includes('bug'),
    'issueList parseado'
  );

  const created = await gm.issueCreate('o/r', {
    title: 'Nuevo',
    body: 'cuerpo',
    labels: ['bug', 'x'],
  });
  ...
  assert(
    postBody.title === 'Nuevo' && postBody.labels.length === 2,
    'payload issueCreate',
...
async function testPRs() {
  ...
  const fake = makeFetch({
    ...
    'POST https://api.github.com/repos/o/r/pulls/12/reviews': {
      status: 201,
      body: {
        id: 555,
        state: 'APPROVED',
        html_url: 'https://github.com/o/r/pull/12#pullrequestreview-555',
      },
    },
  });
  const gm = new GitHubManager({ fetch: fake, token: 't' });
  ...
  const pr = await gm.prCreate('o/r', {
    title: 'Mi PR',
    head: 'feat-x',
    base: 'main',
    body: 'desc',
  ...
  assert(
    revBody.event === 'APPROVE' && revBody.body === 'LGTM',
    'payload review',
...
async function testActions() {
  console.log(C.bold('\n── Test 5: actions_status ───────────────────────────────────────'));
  const fake = makeFetch({
    'GET https://api.github.com/repos/o/r/actions/runs?per_page=5': {
      body: {
        workflow_runs: [
          {
            id: 1,
...
async function testErrors() {
  ...
  let err = null;
  try {
    await gm.repoInfo('o/r');
  } catch (e) {
    err = e;
  }
  assert(
    err && err.status === 401 && /Token inválido/.test(err.message),
    '401 → mensaje claro',
    err?.message
  );
  err = null;
  try {
    await gm.repoInfo('o/missing');
  } catch (e) {
    err = e;
  }
...
async function testValidation() {
  ...
  let threw = false;
  try {
    await gm.repoInfo('solo-un-param');
  } catch (e) {
    threw = /owner\/repo/.test(e.message);
  }
  assert(threw, 'repo sin "/" rechazado');

  threw = false;
  try {
    await gm.prReview('o/r', { pull_number: 5, event: 'LGTM', body: 'x' });
  } catch (e) {
    threw = /event inválido/.test(e.message);
  }
  assert(threw, 'review event inválido rechazado');

  threw = false;
  try {
    await gm.issueCreate('o/r', {});
  } catch (e) {
    threw = /título/.test(e.message);
  }
  ...
  const fake = makeFetch({
    'GET https://api.github.com/user': {
      body: {
        ...
        name: 'Panfilo',
        public_repos: 3,
        html_url: 'https://github.com/panfilo',
      },
    },
  });
  const gm = new GitHubManager({ fetch: fake, token: 't' });
