...
assert(
  !/\p{Extended_Pictographic}/u.test(out1),
  'caso real: cero emojis tras filtrar → ' + JSON.stringify(out1)
);

// banderas, keycaps, secuencias ZWJ
const out2 = strip('hola 🇦🇷 mundo 👍‍🔥 y 5️⃣ fin');
assert(
  !/\p{Extended_Pictographic}/u.test(out2) && !/[\u{1F3FB}-\u{1F3FF}]/u.test(out2),
  'banderas/ZWJ/keycaps eliminadas'
);
