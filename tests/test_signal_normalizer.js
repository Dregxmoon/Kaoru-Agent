...
function testFiltering() {
  ...
  assert(
    normalize('git:redflag', { kind: 'uncommitted', count: 5, message: 'x' }) !== null,
    'git:redflag → candidato'
  );
  assert(normalize('no:existe', {}) === null, 'desconocido sin datos → null (no es señal)');
  assert(
    normalize('os:idle-changed', { idle: true }) === null,
    'contexto (idle) → no es candidato'
  );
...
function testGit() {
  ...
  assert(
    env.signal.costOfIgnore > 0.9,
    'env_unignored → costOfIgnore casi 1',
    `got=${env.signal.costOfIgnore}`
  );
  ...
  assert(
    conflict.signal.severity > 0.7,
    'merge_conflict → severidad alta',
    `got=${conflict.signal.severity}`
  );
  ...
  assert(
    many.urgencia > few.urgencia,
    '30 sin commitear → más urgente que 3',
    `few=${few.urgencia} many=${many.urgencia}`
  );
...
function testSystem() {
  ...
  assert(
    crit.signal.severity > 0.8,
    'batería crítica → severidad alta',
    `got=${crit.signal.severity}`
  );
  ...
  assert(
    low.urgencia < crit.urgencia,
    'battery_low menos urgente que critical',
    `low=${low.urgencia} crit=${crit.urgencia}`
  );
  ...
  assert(
    diskHigh.urgencia > diskLow.urgencia,
    'disco 96% más urgente que 80%',
    `low=${diskLow.urgencia} high=${diskHigh.urgencia}`
  );
  ...
  assert(
    JSON.stringify(again.signal) === JSON.stringify(diskHigh.signal),
    'determinista ante re-emisión'
  );
}
...
function testLsp() {
  ...
  const focused = normalize('lsp:error', {
    file: 'src/a.ts',
    absPath: '/w/src/a.ts',
    errors: [err1],
    count: 1,
  ...
  const unfocused = normalize('lsp:error', {
    file: 'src/b.ts',
    absPath: '/w/src/b.ts',
    errors: [err1],
    count: 1,
  ...
  assert(
    focused.signal.salience === 1,
    'archivo enfocado → saliencia 1',
    `got=${focused.signal.salience}`
  );
  assert(
    focused.signal.actionability > unfocused.signal.actionability,
    'enfocado → más accionable'
  );
  assert(focused.urgencia > unfocused.urgencia, 'enfocado → más urgente');
  assert(
    unfocused.confianza < focused.confianza,
    'no enfocado → menos confianza',
    `f=${focused.confianza} u=${unfocused.confianza}`
  );

  // Más errores → más severo.
  const manyErr = normalize('lsp:error', {
    file: 'src/a.ts',
    errors: [err1, err1, err1, err1],
    count: 4,
    focused: false,
  });
  assert(
    manyErr.signal.severity > focused.signal.severity,
    '4 errores → más severo que 1',
    `1=${focused.signal.severity} 4=${manyErr.signal.severity}`
  );
}
...
function testOthers() {
  ...
  const crash = normalize('os:error-title', {
    title: 'app crashed',
    app: 'node',
    category: 'crash',
  });
  assert(
    crash.signal.salience >= 0.9,
    'error en pantalla → saliencia alta',
    `got=${crash.signal.salience}`
  );
  assert(crash.signal.severity > 0.7, 'crash → severidad alta', `got=${crash.signal.severity}`);

  const stack = normalize('clipboard:copied', {
    kind: 'stacktrace',
    snippet: 'TypeError: x is not a function',
  });
  const url = normalize('clipboard:copied', { kind: 'url', snippet: 'https://example.com' });
  assert(
    stack.urgencia > url.urgencia,
    'stacktrace más urgente que URL',
    `stack=${stack.urgencia} url=${url.urgencia}`
  );
  ...
  const soon = normalize(
    'memory:upcoming-event',
    { content: 'Reunión', when: now + 5 * 60000 },
    { now }
  );
  const later = normalize(
    'memory:upcoming-event',
    { content: 'Reunión', when: now + 5 * 3600000 },
    { now }
  );
  assert(
    soon.urgencia > later.urgencia,
    'evento en 5 min más urgente que en 5 h',
    `soon=${soon.urgencia} later=${later.urgencia}`
  );
...
function testGenerality() {
  ...
  const unknown = normalize('deploy:failed', {
    service: 'api',
  ...
  if (unknown) {
    ...
    assert(
      unknown.signal.severity >= 0.5,
      'payload con error/fail → severidad media-alta',
      `got=${unknown.signal.severity}`
    );
  ...
  assert(
    secret !== null && secret.isCritical === true,
    'payload con secreto → isCritical',
    secret ? `isCritical=${secret.isCritical}` : 'null'
  );

  // Evento con fallo accionable → accionabilidad razonable.
  const actionable = normalize('build:failed', {
    message: 'build failed',
    command: 'npm run build',
  });
  assert(
    actionable !== null && actionable.accionabilidad >= 0.5,
    'con command/file → accionable',
    actionable ? `a=${actionable.accionabilidad}` : 'null'
  );
  ...
  assert(
    learned !== null && learned.tipo === 'test_telemetry_spike',
    'registerProfile → tipo aprendido',
    learned ? learned.tipo : 'null'
  );
  if (learned)
    assert(
      Math.abs(learned.signal.severity - 0.7) < 1e-9,
      'registerProfile → perfil usado',
      `got=${learned.signal.severity}`
    );
}
