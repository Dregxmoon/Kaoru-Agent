...
function stubLLM({ complete } = {}) {
  ...
  LLMProvider.complete = async (...args) =>
    complete
      ? complete(...args)
      : 'Vi que tienes un .env sin ignorar — puedo añadirlo al .gitignore para que no se suba por accidente.';
  return {
    restore: () => {
...
(async () => {
  console.log(
    C.cyan(C.bold('Fase B — E2E: GitWatcher → ProactiveEngine → ProactiveExecutor (git real)'))
  );
  ...
  assert(
    payload.proposal.action?.params?.file === '.env',
    `params resueltos del sensor: file=${payload?.proposal?.action?.params?.file}`
  );
  assert(
    payload.proposal.diff && payload.proposal.diff.includes('+.env'),
    'diff real de la propuesta (solo lectura)'
  );
  assert(
    payload.suggestion && payload.suggestion.length > 5,
    'mensaje del LLM presente (el LLM dio el OK)'
  );

  // 2. El engine registró la acción pendiente.
  assert(
    engine._pendingActions.has(payload.proposal.id),
    'acción pendiente registrada para la decisión'
  );
  ...
  assert(
    result.detail.includes('check-ignore'),
    'detail con la verificación REAL de git check-ignore'
  );
  assert(
    result.proposalId === payload.proposal.id,
    'resultado referenciado al proposalId correcto'
  );
