...
function makeLSPStub({ diagnosticsByFile = {}, defaultDiag = [] } = {}) {
  ...
  return {
    ...
    diagnosticsByFile,
    isRunning: true,
    async openDocument(f) { opened.add(f); },
...
async function testWatcher() {
  ...
  const lsp = makeLSPStub({
    diagnosticsByFile: {
      [badJs]: [
        { code: '1109', message: 'Expression expected.', range: { start: { line: 0, character: 10 } }, severity: 1 },
        { code: '7031', message: 'Not all code paths return a value.', range: { start: { line: 5, character: 0 } }, severity: 1 },
        { code: '80001', message: 'This is just a warning.', range: { start: { line: 0, character: 0 } }, severity: 2 },
      ],
      [goodJs]: [],
    },
  });
  ...
  await watcher.poll();
  assert(watcher.getFocusedFile() === badJs, 'detecta el archivo enfocado desde el título de la ventana');
  assert(watcher.getOpenFiles().includes(badJs), 'el archivo enfocado queda en getOpenFiles()');
  const sig = await p;
  assert(sig && sig.file === 'src/bad.js', 'emite lsp:error con el archivo correcto (relativo al workspace)');
  ...
  assert(sig.errors.every(e => e.severity === 1), 'todos los errores son severidad 1');
  assert(!sig.errors.some(e => e.message.includes('warning')), 'ningún warning entra a la señal');
  assert(sig.languageId === 'javascript', 'el sensor reporta el lenguaje del archivo (.js → javascript)', sig.languageId);
  ...
  lsp.diagnosticsByFile[badJs] = [
    ...
    { code: 'NEW', message: 'Un error nuevo apareció.', range: { start: { line: 8, character: 0 } }, severity: 1 },
  ];
  const p2 = waitForEvent(bus, 'lsp:error');
  await watcher.poll();
  const sig2 = await p2;
  assert(sig2.count === 2, 're-emite al aparecer un error nuevo');
  assert(sig2.errors.some(e => e.code === 'NEW'), 'la señal incluye el error nuevo');
  ...
  assert(watcher.getOpenFiles().length === 0, 'resetWorkspace limpia los archivos abiertos (no mezcla proyectos)');
...
async function testSymbolIndex() {
  ...
  lsp.getDocumentSymbols = async () => [
    { name: 'calcular', kind: 12, selectionRange: { start: { line: 0, character: 0 } }, children: [
      { name: 'interna', kind: 6, selectionRange: { start: { line: 2, character: 2 } }, children: [] },
    ] },
    { name: 'constante', kind: 13, selectionRange: { start: { line: 9, character: 0 } }, children: [] },
  ];
  ...
  assert(syms[0].name === 'calcular' && syms[0].kindName === 'Function', 'símbolo con kindName real (LSP como fuente)');
  assert(syms[1].name === 'interna' && syms[1].kindName === 'Method', 'los hijos se incluyen aplanados');
  assert(typeof syms[0].line === 'number', 'cada símbolo lleva su línea');

  let calls = 0;
  const orig = lsp.getDocumentSymbols;
  lsp.getDocumentSymbols = async () => { calls++; return orig(); };
...
async function testApplyPatch() {
  ...
  assert(prev.diff && prev.diff.includes('-  const x = ;') && prev.diff.includes('+  const x = 1;'), 'preview muestra el diff real');
  ...
  const dup = await executor.preview({ tool: 'apply_patch', params: { file: 'src/errores.js', changes: [{ old: 'const y = ;', new: 'const y = 1;' }] } });
  assert(!dup.ok && dup.reason.includes('no es único'), 'fragmento duplicado → rechazado sin tocar nada');

  // 3c. File fuera del workspace / path traversal → rechazado.
  fs.writeFileSync(file, original);
  const traversal = await executor.preview({ tool: 'apply_patch', params: { file: '../outside.js', changes: [{ old: 'a', new: 'b' }] } });
  assert(!traversal.ok, 'path traversal → rechazado');
  const abs = await executor.preview({ tool: 'apply_patch', params: { file: '/etc/passwd', changes: [{ old: 'a', new: 'b' }] } });
  ...
  fs.writeFileSync(file, original);
  afterDiag = [{ message: 'Un error NUEVO que el parche introdujo.', range: { start: { line: 0, character: 0 } }, severity: 1 }];
  ...
  assert(fs.readFileSync(file, 'utf-8') === original, 'el archivo quedó como estaba tras el rollback');
  ...
  assert(open.ok === false && open.detail.includes('abierto en el editor'), 'guard: no escribe sobre archivo abierto en el editor');
  ...
  const noVerify = new ProactiveExecutor({ getWorkspace: () => ws, getOpenFiles: () => [], getDiagnostics: null });
  const resNoV = await noVerify.execute(goodPatch, { proposalId: 'p-nov' });
  assert(resNoV.ok && resNoV.detail.includes('sin verificación LSP'), 'sin LSP aplica igual y lo dice');

  // 3i. Archivo no existe → rechazado sin mutar.
  const missing = await executor.preview({ tool: 'apply_patch', params: { file: 'src/no-existe.js', changes: [{ old: 'a', new: 'b' }] } });
...
async function testEngineLspError() {
  ...
  const error = { code: '1109', message: 'Expression expected.', range: { start: { line: 1, character: 10 } }, severity: 1 };

  const got = waitForEvent(bus, 'initiative:trigger');
  engine._onLspError({ file: 'src/buggy.js', absPath: file, workspace: ws, errors: [error], focused: true, symbols: [{ name: 'saludar', kindName: 'Function', line: 0 }] });
  ...
  assert(payload.proposal.action.params.changes.length === 1, 'params: cambios generados por el LLM');
  assert(payload.proposal.action.params.changes[0].new.includes('nombre'), 'params: el parche del LLM viaja a la propuesta');
  assert(payload.proposal.action.params.targetErrors.length === 1, 'params: targetErrors para la verificación post-parche');
  assert(payload.proposal.diff && payload.proposal.diff.includes('+'), 'el bubble muestra el diff real');
  ...
  engine.handleDecision({ proposalId: payload.proposal.id, type: 'lsp_error', decision: 'accepted' });
  const done = await execResult;
  assert(done.ok === true && done.detail.includes('verificado con el LSP'), 'aceptar la propuesta aplica el parche y verifica con LSP');
  assert(fs.readFileSync(file, 'utf-8').includes('"hola" + nombre'), 'el archivo real quedó corregido');
  stub.restore();
  ...
  engine2._onLspError({ file: 'src/buggy.js', absPath: file, workspace: ws, errors: [error], focused: true, symbols: [] });
  const payload2 = await p2;
  assert(payload2.proposal && payload2.proposal.action === null, 'parche inválido → propuesta SIN acción');
  ...
  bus.emit('lsp:error', { file: 'src/buggy.js', absPath: file, workspace: ws, errors: [error], focused: true, symbols: [] });
...
async function testPatchLanguage() {
  ...
  const stub = stubLLM({
    complete: async (...args) => {
      ...
      return JSON.stringify({ changes: [{ old: 'function sumar(a, b) {', new: 'function sumar(a, b) {' }] });
    },
  });
  const engine = makeEngine(
    new ProposalStore({ filePath: path.join(tmpRoot, 'feedback-lang.json') }),
    new ProactiveExecutor({ getWorkspace: () => ws, getOpenFiles: () => [], getDiagnostics: async () => [], verifyDelayMs: 5 })
  );
  await engine._generatePatch({
    file: 'src/calc.js', fileType: '.js', absPath: file, workspace: ws,
    errors: [{ code: '7006', message: "Parameter 'a' implicitly has an 'any' type.", line: 0 }],
    symbols: [],
  });
  assert(capturedSystem.includes('El archivo es JavaScript'), 'el prompt de parche dice que es JavaScript');
  assert(/PROHIBIDO usar anotaciones de tipos de TypeScript/.test(capturedSystem), '…prohíbe explícitamente la sintaxis TS en JS');
  assert(!capturedSystem.includes('las anotaciones de tipos (a: number) son válidas'), '…la regla NO dice que las anotaciones valen (eso es solo TS)');
  stub.restore();
  ...
  assert(prev.ok === false && /sintaxis del archivo|sintaxis JS inválida/.test(prev.reason), 'preview rechaza sintaxis TS-en-JS', prev.reason);
  ...
  assert(execRes.ok === false && execRes.rolledBack === true, 'execute → rollback ante sintaxis inválida');
  ...
  const good = await executor.preview({
    ...
    params: {
      file: 'src/calc.js',
      changes: [{ old: 'function sumar(a, b) {', new: '/** @param {number} a @param {number} b */\nfunction sumar(a, b) {' }],
    },
  });
...
async function main() {
  ...
  if (failed === 0) {
    console.log(`  ${C.green('Resultado')}: ${C.green(`${passed} passed`)}  ${C.dim('0 failed')}  / ${total} total`);
  } else {
