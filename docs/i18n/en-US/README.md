...
- **Auditable initiative:** sensor signals are normalized and classified as <code>ACT</code>, <code>QUEUE</code>, <code>DROP</code>, or <code>ESCALATE</code>.
- **Governed actions:** proposals, permission policy, consent, execution, verification, and recovery remain separate stages.
- **Extensible:** MCP servers, local plugins, skills, subagent profiles, and LSP servers.
